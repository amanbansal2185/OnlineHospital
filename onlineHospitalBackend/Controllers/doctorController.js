...
export const deleteDoctor = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "successfully deleted  Doctor" });
  } catch (err) {
  ...
  try {
    ...
    if (query) {
      ...
      doctors = await Doctor.find({ isApproved: "approved" })
        .populate("reviews")
        .select("-password"); // pass empty object and find all the Doctor in the database
    ...
    res
      .status(404)
      .json({ success: false, message: "failed to find  all Doctor " });
  }
};
...
export const getDoctorProfile = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Something went wrong get doctor profile please try again",
    });
  }
};
