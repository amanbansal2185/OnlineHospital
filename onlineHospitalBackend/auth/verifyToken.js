...
const STRIPE_SECRET_KEY =
  "sk_test_51PFFNNSCY92SifIK3jhFRWFQLVxnvlgdLKPVCsMDw4mKMOyZZVzLxZWEfniCOHQ3OILepIqDpcDM1melLy0NC5Wz00KKAoHHgG";
...
// The Authorization header in the HTTP request typically contains a token in the format "Bearer ". The "Bearer" prefix is used to indicate that the token is a
// bearer token, which is a type of token that is used to authenticate and authorize requests.
export const authenticate = async (req, res, next) => {
  ...
  if (!authToken || !authToken.startsWith("Bearer ")) {
    ...
    return res
      .status(401)
      .json({ success: false, message: " no token unauthorized access" });
  }
  try {
    ...
    res
      ...
      .json({
        success: false,
        message: " no token (invalid token )catch block unauthorized access",
      });
  }
};
...
export const restrict = (roles) => async (req, res, next) => {
  ...
  if (!roles.includes(user?.role)) {
    return res
      .status(401)
      .json({ success: false, message: "unauthorized role for access" });
  }
  next();
};
